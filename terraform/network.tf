resource "google_compute_network" "avalanche" {
  name                    = "avalanche-${local.workspace_name}"
...
resource "time_sleep" "create-subnetworks" {
  create_duration = "30s"
  depends_on      = [google_compute_network.avalanche]
}

data "google_compute_subnetwork" "region" {
  name       = google_compute_network.avalanche.name
  depends_on = [time_sleep.create-subnetworks]
}

resource "google_compute_router" "nat" {
  name    = "avalanche-${local.workspace_name}-nat"
